...
export default function FortunePanel({mode,requestId,close,onCollection,onOpenClip,onAnother}:{mode:'fortune'|'paperclip';requestId:string;close:()=>void;onCollection:(count:number)=>void;onOpenClip:()=>void;onAnother:()=>void}){
 ...
 return <Modal title="" eyebrow={mode==='paperclip'?'Your paper clip collection':'A little note for you'} ariaLabel={mode==='paperclip'?'Saved fortune notes':'Fortune cookie'} close={close} wide={mode==='paperclip'} className={mode==='fortune'?'fortune-game':'fortune-archive'}>
  {busy?<div className="fortune-loading"><img className="fortune-cookie" src="/textures/fortune-cookie.png" alt="" width={280} height={210}/><p>{mode==='fortune'?'Cracking your cookie…':'Gathering your notes…'}</p></div>:error?<div className="empty"><p role="alert">{error}</p><button className="gold-button" disabled={retryAfter>0} onClick={()=>setRetry(x=>x+1)}><RotateCcw size={15}/>{retryAfter>0?`Try again in ${retryAfter}s`:'Try again'}</button></div>:mode==='fortune'?<>
   {note?<FortuneCrack key={requestId+'-'+retry} note={note} onRevealed={finishReveal}/>:exhausted?<div className="empty"><Cookie size={38}/><p>Every cookie, opened.<br/>Every good word is waiting on your paper clip.</p></div>:<div className="empty"><p>This note was removed. Open another cookie below.</p></div>}
   <p className="sr-only" role="status">{note?(revealed?'Your note is ready.':'Opening your cookie…'):exhausted?'Your collection is complete.':'This note was removed.'}</p>
