...
export default function FortunePanel({mode,requestId,close,onCollection,onOpenClip,onAnother}:{mode:'fortune'|'paperclip';requestId:string;close:()=>void;onCollection:(count:number)=>void;onOpenClip:()=>void;onAnother:()=>void}){
 const [note,setNote]=useState<SavedFortune|null>(null),[saved,setSaved]=useState<SavedFortune[]>([]),[busy,setBusy]=useState(true),[error,setError]=useState(''),[exhausted,setExhausted]=useState(false),[retry,setRetry]=useState(0),[revealed,setRevealed]=useState(false);
 useEffect(()=>{let active=true;setBusy(true);setError('');setNote(null);setExhausted(false);setRevealed(false);
 ...
 useEffect(()=>{if(busy||!note)return;const reduced=window.matchMedia('(prefers-reduced-motion: reduce)').matches;const timer=setTimeout(()=>setRevealed(true),reduced?0:1900);return()=>clearTimeout(timer)},[busy,note]);
 return <Modal title={mode==='paperclip'?'A few good words.':'A little good fortune.'} eyebrow={mode==='paperclip'?'THE PAPER CLIP':'PANDA EXPRESS · AFTER DINNER'} close={close} wide={mode==='paperclip'}>
  {busy?<div className="fortune-loading"><img className="fortune-cookie" src="/textures/fortune-cookie.png" alt="" width={280} height={210}/><p>{mode==='fortune'?'Cracking your cookie…':'Gathering your notes…'}</p></div>:error?<div className="empty"><p role="alert">{error}</p><button className="gold-button" onClick={()=>setRetry(x=>x+1)}><RotateCcw size={15}/>Try again</button></div>:mode==='fortune'?<>
   {note?<FortuneCrack key={requestId} note={note}/>:exhausted?<div className="empty"><Cookie size={38}/><p>All 200 cookies, opened.<br/>Every good word is waiting on your paper clip.</p></div>:null}
   ...
   <div className="fortune-actions"><button className="secondary-button" disabled={!!note&&!revealed} onClick={onOpenClip}><Paperclip size={16}/>Your notes · {saved.length}</button>{saved.length<fortunes.length&&<button className="gold-button" disabled={!!note&&!revealed} onClick={onAnother}>One more cookie</button>}</div>
