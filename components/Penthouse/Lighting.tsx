...
export default function Lighting({onReady,lampOn,skyMode}:{onReady:()=>void;lampOn:boolean;skyMode:SkyMode}){
 ...
 return <><ambientLight ref={ambient} intensity={.16} color="#d8e3e5"/><hemisphereLight ref={hemi} args={['#b6d0e7','#6e5844',.48]}/><directionalLight ref={key} position={[1,3.2,4]} intensity={.85} color="#ffe2b6" castShadow shadow-mapSize={[2048,2048]} shadow-camera-left={-8} shadow-camera-right={8} shadow-camera-top={8} shadow-camera-bottom={-8} shadow-bias={-.0003} shadow-normalBias={.015} shadow-radius={4}/><directionalLight position={[-2,4.8,-6]} intensity={skyMode==='day'?1.8:skyMode==='sunset'?.8:.04} color={daylight.color} castShadow shadow-mapSize={[2048,2048]} shadow-camera-left={-7} shadow-camera-right={7} shadow-camera-top={7} shadow-camera-bottom={-7} shadow-bias={-.0002} shadow-normalBias={.015} shadow-radius={4}/><pointLight ref={fillA} position={[-2,2.5,-1]} intensity={2.2} color="#ffd491" distance={9}/><pointLight ref={fillB} position={[3,2.5,1]} intensity={1.6} color="#efcca3" distance={8}/></>;
}
