...
import { floorPlankColors } from '@/lib/room-finishes';
...
export default function InteriorEnvelope(){return <group name="interior-shell">
 ...
 {Array.from({length:25},(_,i)=><group key={i}>{Array.from({length:8},(_,j)=><mesh key={j} position={[-4.8+i*.4,.06,4.67+j*2.34]} receiveShadow><boxGeometry args={[.385,.025,2.325]}/><Surface color={floorPlankColors[(i+j*2)%floorPlankColors.length]} kind="floor"/></mesh>)}</group>)}
