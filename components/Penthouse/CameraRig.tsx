...
function Controller({focus,reset,interior}:{focus:Focus;reset:number;interior:boolean}){
 ...
 return <OrbitControls ref={controls} makeDefault enablePan={false} enableDamping dampingFactor={.08} minPolarAngle={interior?.9:.62} maxPolarAngle={interior?1.6:1.24} minAzimuthAngle={interior?-.62:.12} maxAzimuthAngle={interior?.62:1.25} minZoom={baseZoom*.55} maxZoom={baseZoom*3.2} minDistance={interior?1.4:0} maxDistance={interior?24:30} onChange={contain} onStart={()=>transition.current=false}/>;
}
