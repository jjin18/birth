...
export default function Backdrop({interior,city,mode,onUnavailable}:{interior:boolean;city:number;mode:SkyMode;onUnavailable:(unavailable:boolean)=>void}){
 const [loaded,setLoaded]=useState<{texture:THREE.Texture;city:City['id'];mode:SkyMode}|null>(null);
 ...
 useEffect(()=>{
  ...
  loader.load(path,next=>{
   ...
   const old=current.current;current.current=next;setLoaded({texture:next,city:cities[city].id,mode});old?.dispose();
  },undefined,()=>{if(active)onUnavailable(true)});
  return()=>{active=false};
 },[path,city,mode,onUnavailable]);
 // Frame the decoded image, not the requested city: slow loads keep the old
 // skyline completely stationary until its replacement is ready to render.
 useLayoutEffect(()=>{if(loaded){const crop=skylineCrop(loaded.city,loaded.mode,interior);loaded.texture.repeat.set(1,crop.height);loaded.texture.offset.set(0,crop.bottom)}},[loaded,interior]);
