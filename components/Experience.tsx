...
import { RotateCcw, X, MousePointer2, ArrowUpRight, Heart, Image as ImageIcon, Swords, BedDouble, LampFloor, Dog, Cookie, Paperclip, ScanEye } from 'lucide-react';
...
export default function Experience(){
 ...
 useEffect(()=>{const tick=()=>{const instant=new Date();setNow(instant);setTime(new Intl.DateTimeFormat('en',{timeZone:cities[city].zone,hour:'2-digit',minute:'2-digit',hour12:false}).format(instant))};tick();const timer=setInterval(tick,30000);const refresh=()=>{if(document.visibilityState==='visible')tick()};document.addEventListener('visibilitychange',refresh);window.addEventListener('focus',tick);return()=>{clearInterval(timer);document.removeEventListener('visibilitychange',refresh);window.removeEventListener('focus',tick)}},[city]);
 ...
 return <main className="experience" data-sky-mode={skyMode} data-sky-choice={skyChoice} data-city={cities[city].id} data-interior={interior}>
  ...
  <button className="view-toggle" onClick={()=>{setInterior(value=>!value);home()}}><ScanEye size={15}/>{interior?'Room overview':'Step inside'}</button>
  ...
  <SkyControls choice={skyChoice} mode={skyMode} city={cities[city].name} onChange={setSkyChoice} unavailable={skyUnavailable}/>
