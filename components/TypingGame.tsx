...
export default function TypingGame({ close }: { close: () => void }) {
  ...
  const rating = typingRating(stats.wpm, stats.accuracy, run.startedAt !== null);
  const complete = run.text === passage.text;
  ...
  useEffect(() => {
    ...
    const result = { text: run.text, attempts: run.attempts, mistakes: run.mistakes, elapsed: Math.round(run.finishedAt - run.startedAt) };
  ...
  return <Modal title="" eyebrow="Typing speedrun" ariaLabel="Typing speedrun" close={close} className="typing-panel" headerActions={<>
    ...
    <dl className="typing-stats">
      <div><dt>Time left</dt><dd>{stats.remaining}<small>s</small></dd></div>
      <div><dt>Speed</dt><dd>{stats.wpm}<small>WPM</small></dd></div>
      <div><dt>Accuracy</dt><dd>{stats.accuracy}<small>%</small></dd></div>
      <div title="A game rating based on speed and accuracy"><dt>Rating</dt><dd className="typing-rating">{rating}</dd></div>
    </dl>
    <p id="typing-instructions" className="typing-sr-only">Type the text below, including capitals and punctuation. The {ROUND_MS / 1000}-second timer starts with your first letter. Backspace corrects mistakes.</p>
    ...
    <div className="typing-footer">
      <p id="typing-feedback" className="typing-feedback" role="status">{notice || (done ? (complete ? 'Lore mastered.' : "Time's up!") : hasErrors ? 'Fix the red letters.' : '')}{done&&<span className="typing-sr-only"> {stats.wpm} words per minute, {stats.accuracy}% accuracy. Rating: {rating}.</span>}</p>
