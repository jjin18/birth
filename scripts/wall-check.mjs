...
const get=async()=>await (await fetch(origin+'/api/wall',{headers:{Cookie:cookie}})).json();
const send=(path,value,method='POST',headers={})=>fetch(origin+path,{method,headers:{Origin:origin,Cookie:cookie,'Content-Type':'application/json',...headers},body:JSON.stringify(value)});
const upload=(bytes,headers={})=>fetch(origin+'/api/wall/upload',{method:'POST',headers:{Origin:origin,Cookie:cookie,'Content-Type':'image/jpeg',...headers},body:bytes});
async function unlock(){const r=await send('/api/wall/session',{passcode});assert.equal(r.status,200);cookie=r.headers.get('set-cookie').split(';')[0];assert.match(r.headers.get('set-cookie'),/HttpOnly; SameSite=Strict/)}
try{
 await start('');assert.equal((await get()).editingEnabled,false);assert.equal((await send('/api/wall/session',{passcode})).status,503);await stop();
 await start();const baseline=await get();assert.equal(baseline.entries.length,11);assert.equal(baseline.canEdit,false);assert(baseline.entries.some(p=>p.id==='dinner'));assert(!baseline.entries.some(p=>p.id==='dinner-close'));
 ...
 assert.equal((await fetch(origin+photo.thumbnail)).status,404,'draft image must not be public');
 assert.equal((await fetch(origin+photo.thumbnail,{headers:{Cookie:cookie}})).status,200);
 ...
 console.log('PASS: 11 seeds, passcode/cookie/CSRF guards, metadata date, manual date, text-only notes, idempotent adds, version conflicts, durable edits/photos, sanitized bounded uploads, private drafts, quotas, expiry, passcode rotation, and untouched fortunes.');
