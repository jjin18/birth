...
const {createMatch,stepMatch,poseFor,IDLE_INPUT,CONTROLS,ARENA,PUNCH_DAMAGE,ROUND_SECONDS}=await import('data:text/javascript;base64,'+Buffer.from(bundle.outputFiles[0].text).toString('base64'));
...
let m=createMatch();
assert.equal(ROUND_SECONDS,30);assert.equal(m.time,30);
const timeoutMatch=createMatch('fight');for(let i=0;i<2999;i++)stepMatch(timeoutMatch,[input(),input()],.01);
assert.equal(timeoutMatch.phase,'fight');stepMatch(timeoutMatch,[input(),input()],.02);assert.equal(timeoutMatch.phase,'finished');assert.equal(timeoutMatch.time,0);assert.equal(timeoutMatch.result,'Draw');
assert.equal(createMatch('ready').time,30,'rematches reset to 30 seconds');
...
assert.equal(m.time,30,'ready countdown does not use fighting time');
