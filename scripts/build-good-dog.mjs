...
export async function buildGoodDogOffline(){
 ...
 const bundle=await build({stdin:{contents:`import {mountGoodDog} from './lib/good-dog/ui';mountGoodDog(document.getElementById('game'),${JSON.stringify(assets)});`,resolveDir:process.cwd(),loader:'ts'},bundle:true,write:false,minify:true,format:'iife',target:'es2020'});
 ...
 if(Buffer.byteLength(html)>370000)throw Error('Offline dog game exceeded its 370 KB budget');
