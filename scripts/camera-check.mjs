...
for(const [width,height] of [[1280,850],[390,844]]){const opening=roomOpening(width,height);assert(opening.position[1]<cameraBounds.maxY);assert.deepEqual(opening.position,width<650?[3.9,2.4,6.1]:[4.4,2.35,5.4],'restore the previous close opening');assert.equal(opening.fov,width<650?78:59)}
...
try{
 ...
 if(found){const points=await page.evaluate(()=>{window.__cameraTestActive=false;return window.__cameraPositions});assert(points.length>10);for(const [x,y,z] of points){assert(y<=5.20001&&y>=.64999,'camera remains below ceiling and above floor');assert(x>=-4.55001&&x<=4.55001&&z>=-2.95001&&z<=8.70001,'camera remains inside walls')}console.log('Validated',points.length,'live camera positions')}
 ...
 assert.deepEqual(errors,[]);console.log('PASS: restored close desktop/mobile opening, camera containment, browser interactions, no runtime errors.');
