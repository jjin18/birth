...
`,resolveDir:process.cwd(),loader:'tsx'},bundle:true,write:false,outdir:'.local-data/arcade-preview',format:'iife',platform:'browser',jsx:'automatic',define:{'process.env.NODE_ENV':'"production"'},plugins:[{name:'local-result-fixtures',setup(build){
 build.onResolve({filter:/^@\/lib\/fighter-game$/},()=>({path:'fighter-fixture',namespace:'fixture'}));
 build.onLoad({filter:/.*/,namespace:'fixture'},()=>({loader:'ts',resolveDir:process.cwd(),contents:`
export * from './lib/fighter-game';import {createMatch as original} from './lib/fighter-game';
export function createMatch(phase='lobby'){const match=original(phase);if(phase==='ready'&&['/jia-wins','/ryan-wins','/draw','/final-seconds'].includes(location.pathname)){match.ready=.05;match.time=location.pathname==='/final-seconds'?10:.05;if(location.pathname==='/jia-wins')match.fighters[1].hp=80;if(location.pathname==='/ryan-wins')match.fighters[0].hp=80;}return match;}
`}));
}}]});
...
createServer(async(req,res)=>{
 ...
 if(['/','/poses','/jia-wins','/ryan-wins','/draw','/final-seconds'].includes(path)){res.setHeader('Content-Type','text/html; charset=utf-8');res.end('<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>Arcade local QA</title><link rel="stylesheet" href="/bundle.css"><div id="root"></div><script src="/bundle.js"></script>');return;}
...
}).listen(Number(process.env.ARCADE_QA_PORT||3105),'127.0.0.1',()=>console.log('Arcade QA ready (local-only result fixtures)'));
