...
assert.equal(photos.length,12);assert.equal(new Set(photos.map(p=>p.id)).size,12);
...
assert.equal(photos.filter(p=>p.date==='2026-09-11').length,4);
let bytes=0;
...
console.log(`PASS: 12 photos, five confirmed date groups, three undated, sanitized metadata, lazy thumbnails/full-size viewer, ${bytes} bytes; one 384×256 board atlas (0.375 MiB RGBA).`);
